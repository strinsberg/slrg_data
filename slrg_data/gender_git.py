...
def main(users_file=None, db_login=None, db_passwd=None,
    ...
    if users_file is None:
        users_file = input("Ghtorrent results file: ")
    ...
    # Will need to add github collection info object
    ...
    gender_collector = None  # Create a git gender collector class
    _add_gender(user_list, gender_collector, gender_file, missing_file)


def _add_gender(user_list, gender_collector, gender_file, miss_file):
    gender = []
    missing = []

    try:
        for i, user in enumerate(user_list):
            # Setup a class to use the gitcollector gender and name functions
            if _has_gender(user):
                gender.append(user)
